...
# Model (Peewee)
# Ogni 'cart' rappresenta un dato acquisito riguardante un carrello
class Cart(Model):
    "Model di 'peewee' dei cart sul database"
...
with open(db_folder + db_txt_folder + dataset_name + ".txt", "rb") as features:
    ...
    for line in database:
        ...
        c_timestamp = datetime.datetime.strptime(str(np.array(f_info[3])), "%Y-%m-%d %H:%M:%S")
        c_x = float(f_info[4])
        c_y = float(f_info[5])

        # Istanza del model
        cart = Cart(id=c_id, x=c_x, y=c_y, tag_id=c_tag_id, timestamp=c_timestamp)
